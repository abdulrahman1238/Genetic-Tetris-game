...
def main(chromosome, evaluate_fitness, num_iteration, generation=1, i=1, test = False):
    ...
    if (MANUAL_GAME):
        final_score = run_game(final_score, chromosome, evaluate_fitness, num_iteration, generation, i,test)

   # print(f'\n\n\nFinal Score = {final_score}')
    return final_score

score_piece = 0
def run_game(final_score, chromosome, evaluate_fitness, num_iteration, generation, i, test):
    # Setup variables
    ...
    while iteration < num_iteration:
        if (falling_piece == None):
            ...
            if (not is_valid_position(board, falling_piece)):
                ...
                print(f'{i}-{chromosome},generation= {generation} last iteration= {iteration+1} andfinal score= {final_score} ')
                return final_score
        ...
        if flag:
            all_pieces, fitness_values = rating(board, falling_piece, chromosome, evaluate_fitness)
          #  print((fitness_values))
           # print(len(all_pieces))
            if len(all_pieces) != 0:
                fitness_values, all_pieces = zip(*sorted(zip(fitness_values, all_pieces), key=lambda x: x[0]))
                falling_piece = all_pieces[-1]
                score_piece = fitness_values[-1]

        # print((fitness_values))
            #for x in all_pieces:
             # print(f'x final= {x["x"]}     ')
            #falling_piece['y'] = -2                             # control speed
            ...
            if (not is_valid_position(board, falling_piece, adj_Y=1)):
                ...
                if flag and test:
                    ...
                    for x2 in range(0, BOARDWIDTH):
                        ...
                        sum_heights += b[2]
                        column_high.append(b[2])
                    max_high = max(column_high)
                    bumpiness = 0
                    for z in range(len(column_high) - 1):
                        bumpiness += abs(column_high[z] - column_high[z + 1])

                    with open('values_optimal1', 'a') as f:
                        f.write(f"{iteration+1}-\ncomplete line = {score_lines[num_removed_lines]}\ntotal holes = {total_holes}\ntotal blocking block = {total_blocking_block}\nsum highs = {sum_heights}\n max highs = {max_high}\n bumpiness = {bumpiness}\n sore = {score_piece}\n\n")
        ...
        draw_board(board)
        draw_status(score, level,iteration+1)
        ...
        if iteration == num_iteration:
            print(
                f'{i}-{chromosome},generation= {generation} last iteration= {iteration} and final score= {final_score} ')
            return final_score


##############################################################################
# GAME FUNCTIONS falling_piece
##############################################################################
def rating(board, falling_piece, chromosome, evaluate_fitness):
    k180 = 0
    all_pieces = []
    fitness_values = []
    score_lines = [0, 40, 120, 300, 1200]
    while (is_valid_position(board, falling_piece)) and (k180 < 4):



        for i in range(0, 10):
            new_falling_piece = copy.deepcopy(falling_piece)
            while is_valid_position(board, new_falling_piece, adj_X=-1, adj_Y=1):
                new_falling_piece['x'] -= 1

            new_falling_piece['x'] += i
        #    print(f'x old{i} = {falling_piece["x"]}       y = {falling_piece["y"]}')
        #    print(f'x new{i} = {new_falling_piece["x"]}       y = {new_falling_piece["y"]}')

            if not is_valid_position(board, new_falling_piece, adj_X=0, adj_Y=1):
                continue
            ...
            while is_valid_position(board, new_falling_piece, adj_Y=1):
                new_falling_piece['y'] += 1

            add_to_board(new_board, new_falling_piece)

            num_removed_lines = remove_complete_lines(new_board)
            line_complete = score_lines[num_removed_lines]
            column_high = []
            for x2 in range(0, BOARDWIDTH):
            ...
            all_pieces.append(new_falling_piece)
            score_piece = evaluate_fitness(chromosome, [line_complete, -total_holes, -sum_heights, -max_high, -total_blocking_block, -bumpiness])
            fitness_values.append(score_piece)
            # new_falling_piece['y'] = -2

        falling_piece['rotation'] = (falling_piece['rotation'] + 1) % len(PIECES[falling_piece['shape']])
        k180 += 1

    return all_pieces, fitness_values
...
def calc_level_and_fall_freq(score):
    ...
    fall_freq = 0.27 - (level * 0.02*100000)
...
def draw_status(score, level,pieces_played):
    """Draw status"""

    # Draw the score text
    score_surf = BASICFONT.render('Score: %s' % score, True, TEXTCOLOR)
    score_rect = score_surf.get_rect()
    score_rect.topleft = (WINDOWWIDTH - 170, 80)
    DISPLAYSURF.blit(score_surf, score_rect)

    # draw the level text
    levelSurf = BASICFONT.render('Level: %s' % level, True, TEXTCOLOR)
    levelRect = levelSurf.get_rect()
    levelRect.topleft = (WINDOWWIDTH - 170, 110)
    DISPLAYSURF.blit(levelSurf, levelRect)

    pieces_surf = BASICFONT.render('Pieces Played: %s' % pieces_played, True, TEXTCOLOR)
    pieces_rect = pieces_surf.get_rect()
    pieces_rect.topleft = (WINDOWWIDTH - 170, 140)
...
def draw_next_piece(piece):
    ...
    next_rect.topleft = (WINDOWWIDTH - 170, 170)
    DISPLAYSURF.blit(next_surf, next_rect)

    # draw the "next" piece
    draw_piece(piece, pixelx=WINDOWWIDTH-170, pixely=170)
